	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td style='width:100px;' dir='"+dir+"'>";
				str +="<span style='vertical-align: middle;'>&nbsp;&nbsp;每页</span><select class='ui-pg-selbox' role='listbox'>";
				for(i=0;i<ts.p.rowList.length;i++){
					str +="<option role=\"option\" value=\""+ts.p.rowList[i]+"\""+((ts.p.rowNum === ts.p.rowList[i])?" selected=\"selected\"":"")+">"+ts.p.rowList[i]+"</option>";
				}
				str +="</select><span style='vertical-align: middle;'>条</span></td>";
			}
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' style='vertical-align: middle;' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span style=\"vertical-align: middle;\" id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir==="rtl") { po.reverse(); }
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><i class='fa fa-step-backward'></i></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><i class='fa fa-backward'></i></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><i class='fa fa-forward'></i></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><i class='fa fa-step-forward'></i></td>";
...
(function($){
	...
	$.extend($.jgrid,{
	    defaults : {
	        ...
	        emptyrecords: "无数据显示",
	        loadtext: "读取中...",
	        pgtext : " {0} <span style='vertical-align: middle;'>共</span> {1} <span style='vertical-align: middle;'>页</span>"
	    },
	    search : {
	        caption: "搜索...",
	        Find: "查找",
