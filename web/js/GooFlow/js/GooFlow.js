...
//初始化区域图的对象
    ...
    this.$activityType="";//当前审批类型
    this.$activityTypeName="";//当前审批类型名称
    this.$nodeToLineRelaion={};//增加节点与线的关系
    this.$lineData = {};
    this.$lineCount = 0;
    this.$nodeData = {};
    this.$nodeCount = 0;
    ...
    if (property.haveTool) {
        ...
        if (property.toolBtns && property.toolBtns.length > 0) {
            ...
            for (var i = 0; i < property.toolBtns.length; ++i) {
                tmp += "<a href='javascript:void(0)' type='" + property.toolBtns[i] + "' activityType='" + property.toolApproveType[i] + "'  activityTypeName='" + property.toolApproveTypeName[i] + "' id='" + this.$id + "_btn_" + property.toolBtns[i].split(" ")[0] + "' class='GooFlow_tool_btn'><i class='ico_" + property.toolBtns[i] + "'/></a>";//加入自定义按钮
            }
        ...
        this.$tool.on("click", {inthis: this}, function (e) {
            ...
            //增加两个属性 beign
            var This = e.data.inthis;
            This.$activityType=$(tar).attr("activityType");
            This.$activityTypeName=$(tar).attr("activityTypeName");
            //增加两个属性 end
    ...
    if (this.$editable) {
        this.$workArea.on("click", {inthis: this}, function (e) {
            ...
            This.addNode(This.$id + "_node_" + This.$max, {
                ...
                isSaved:false   //增加是否保存的属性 false or true
            });
            This.$max++;
        });
        //划线或改线时用的绑定
        ...
        this.onItemDel = null;
        //格式function(id，type)：id是单元的唯一标识ID,type是单元的种类,有"node","line","area"三种取值 删除方法回调事件
        this.onItemDeleteCallBack=null;
...
GooFlow.prototype = {
    ...
    delNode: function (id) {
        ...
        //删除数据回调
        if (this.onItemDeleteCallBack != null){
            this.onItemDeleteCallBack(id, "node");
        }
    },
    //设置流程图的名称
...
///////////以下为有关画线的方法
    ...
    addLine: function (id, json) {
        ...
        //增加两个属性 start
        this.$lineData[id].fromNodeName = this.$nodeData[json.from].name;
        this.$lineData[id].toNodeName = this.$nodeData[json.to].name;
        if(this.$nodeToLineRelaion[json.from]){
            this.$nodeToLineRelaion[json.from] = this.$nodeToLineRelaion[json.from]+"#"+id;
        }else{
            this.$nodeToLineRelaion[json.from] = id;
        }

        if(this.$nodeToLineRelaion[json.to]){
            this.$nodeToLineRelaion[json.to] = this.$nodeToLineRelaion[json.to]+"#"+id;
        }else{
            this.$nodeToLineRelaion[json.to] = id;
        }
        //增加两个属性 end
    ...
    delLine: function (id) {
        ...
        //删除数据回调
        if (this.onItemDeleteCallBack != null){
            this.onItemDeleteCallBack(id, "line");
        }
    },
    //变更连线两个端点所连的结点
    ...
    moveLinePoints: function (lineId, newStart, newEnd, noStack) {
        ...
        if (newStart != null && newStart != "") {
            ...
            this.$lineData[lineId].fromNodeName = this.$nodeData[newStart].name;//新增节点name属性
        }
        if (newEnd != null && newEnd != "") {
            this.$lineData[lineId].to = newEnd;
            this.$lineData[lineId].toNodeName = this.$nodeData[newEnd].name;//新增节点name属性
        }
        //增加节点与线的关系，一个节点可能关联多跟线 start
        if(this.$nodeToLineRelaion[json.from]){
            this.$nodeToLineRelaion[json.from] = this.$nodeToLineRelaion[json.from]+"#"+id;
        }else{
            this.$nodeToLineRelaion[json.from] = id;
