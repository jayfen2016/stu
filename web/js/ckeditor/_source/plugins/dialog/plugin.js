...
(function()
{
	...
	(function()
	{
		var notEmptyRegex = /^([a]|[^a])+$/,
            ...
            nodeListRegex = /^\[([^,]+,)*[^,]+\]$/,
            fieldsRegex =/^([^,*],)*[^,*]$/,
            columnsRegex = /^[^,]+?,[^,]+?,[^,]*,(true|false),(true|false)$/,
            integerNotNullRegex = /^\d+$/,
            columsNotEmptyRegex =/^([^:]+:%[^%]+%,)*[^:]+:%[^%]+%$/,
            statisticsFieldRegex=/^([^:]*?:[^,]*?,)*[^:]*?:[^,]+$/,
            //#endregion Beyondbit
		...
		CKEDITOR.dialog.validate =
		{
            ...
            integerNotEmpty : function( msg )
			{
				return this.regex( integerNotNullRegex, msg );
			},
			'number' : function( msg )
			{
            ...
            //#region Beyondbit
            columns: function (msg) {
		        return this.regex(columsRegex, msg);
		    },
            fields:function(msg)
            {
                return this.functions( 
                    function( val ){                                
                        return  !notEmptyRegex.test(CKEDITOR.tools.trim( val )) 
                                || fieldsRegex.test( CKEDITOR.tools.trim( val ) );
                               }, msg );   
            },
            statisticsField:function(msg)
            {
                 return this.functions( 
                    function( val ){                                
                        return  !notEmptyRegex.test(CKEDITOR.tools.trim( val )) 
                                || statisticsFieldRegex.test( CKEDITOR.tools.trim( val ) );
                               }, msg );    
            },
            columnsNotEmpty: function (msg) {
		        return this.regex(columsNotEmptyRegex, msg);
		    },
            nodeList: function (msg) {
		        return this.regex(nodeListRegex, msg);
		    },
            nodeListOrEmpty: function (msg) {
                return this.functions( 
                    function( val ){                                
                        return  !notEmptyRegex.test(CKEDITOR.tools.trim( val )) 
                                || nodeListRegex.test( CKEDITOR.tools.trim( val ) );
                               }, msg );             
		    }
            //#endregion 
		};
