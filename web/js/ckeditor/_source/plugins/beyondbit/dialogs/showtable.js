...
(function () {
    ...
    function tableDialog(editor, command) {
        ...
        return {
            title: "显示列表" + editor.lang.table.title,
            minWidth: 410,
            ...
            onShow: function () {
                ...
                if (command == 'formshowtableProperties') {
            ...
            onOk: function () {
                ...
                if (!this._.selectedElement) {
                    table.setAttribute('showFlag', 'true');
							...
							[
								   {
								       ...
								       accessKey: 'S',
								       value: "",
								       validate: CKEDITOR.dialog.validate.notEmpty("SQL语句不能为空！"),
								       setup: function (element) {
								           this.setValue($(element.$).attr("sql"));
								       },
								       commit: function (infordata, data) {
								           var element = data.$;
								           var value = this.getValue();
								           $(element).attr("sql", value);
								       }
								   },
                                     {
                                         type: 'hbox',
						...
						[
                            {
                                ...
                                accessKey: 'S',
                                setup: function (element) {
                                    this.setValue($(element.$).attr("trClass"));
                                },
                                commit: function (infordata, data) {
                                    var element = data.$;
                                    var value = this.getValue();
                                    $(element).attr("trClass", value);
                                }
                            },
                            {
                                id: 'trStyle',
                                ...
                                value: "",
                                setup: function (element) {
                                    this.setValue($(element.$).attr("trStyle"));
                                },
                                commit: function (infordata, data) {
                                    var element = data.$;
                                    var value = this.getValue();
                                    $(element).attr("trStyle", value);
                                }
                            }
                        ]
                                     },

                            {
                                ...
                                accessKey: 'S',
                                setup: function (element) {
                                    this.setValue($(element.$).attr("where"));
                                },
                                commit: function (infordata, data) {
                                    var element = data.$;
                                    var value = this.getValue();
                                    $(element).attr("where", value);
                                }
                            },
                                      {
                                          id: 'showColumns',
                                          type: 'text',
                                          label: "*显示字段[格式：表字段,标题,50%,true,false,false(带附件) 注：50%为宽度]",
                                          'default': '',
                                          accessKey: 'M',
                                          items: [],
                                          validate: CKEDITOR.dialog.validate.notEmpty("显示字段不能为空！"),
                                          setup: function (element) {
                                              this.setValue($(element.$).attr("showColumns"));
                                          },
                                          commit: function (infordata, data) {
                                              var element = data.$;
                                              var value = this.getValue();
                                              $(element).attr("showColumns", value);
                                          }
                                      },
                                    {
                                        type: 'hbox',
						...
						[
                            {
                                ...
                                value: "checked",
                                setup: function (element) {
                                    if ($(element.$).attr("showtitle") == "true")
                                        this.setValue($(element.$).attr("showtitle"));
                                },
                                commit: function (infordata, data) {
                                    var element = data.$;
                                    var value = this.getValue();
                                    if (value)
                                        $(element).attr("showtitle", "true");
                                    else
                                        $(element).attr("showtitle", "false");
                                }
                            }, {
                                id: 'showBorder',
                                type: 'checkbox',
                                ...
                                value: "checked",
                                setup: function (element) {
                                    if ($(element.$).attr("showBorder") == "true")
                                        this.setValue($(element.$).attr("showBorder"));
                                },
                                commit: function (infordata, data) {
                                    var element = data.$;
                                    var value = this.getValue();
                                    if (value)
                                        $(element).attr("showBorder", "true");
                                    else
                                        $(element).attr("showBorder", "false");
                                }
                            }
                        ]
                                    },
						...
						[
                              {
                                  ...
                                  value: "checked",
                                  setup: function (element) {
                                      this.setValue($(element.$).attr("tdClass"));
                                  },
                                  commit: function (infordata, data) {
                                      var element = data.$;
                                      $(element).attr("tdClass", this.getValue());
                                  }
                              },
                                {
                                    id: 'tdStyle',
                                    ...
                                    accessKey: 'M',
                                    setup: function (element) {
                                        this.setValue($(element.$).attr("tdStyle"));
                                    },
                                    commit: function (infordata, data) {
                                        var element = data.$;
                                        var value = this.getValue();
                                        $(element).attr("tdStyle", value);
                                    }
                                }
                        ]
                                       },
    ...
    CKEDITOR.dialog.add('formshowtable', function (editor) {
        return tableDialog(editor, 'formshowtable');
    });
    CKEDITOR.dialog.add('formshowtableProperties', function (editor) {
        return tableDialog(editor, 'formshowtableProperties');
    });
})();
